...
function banCheck ( nick, ip, uname, serial )
	...
	elseif string.find ( nick, "'" ) then
		...
		if playerUID[nick] then
			local result = dbPoll ( dbQuery ( handler, "SELECT ??, ??, ??, ?? FROM ?? WHERE ??=?", "Abgelaufen", "STime", "Grund", "AdminUID", "ban", "UID", playerUID[nick] ), -1 )
			...
			if bool and bantime then
				...
				elseif getPlayerWarnCount ( nick ) >= 3 then
					checkExpiredWarns ( getPlayerFromName ( nick ) )
...
function register_func ( player, passwort, bday, bmon, byear, geschlecht, bcode, email, werber )
    if player == client then
		...
		--	local email = tostring(email)
            ...
            if not dbExist ( "coins", "Name LIKE '"..pname.."'") then
                local result = dbExec ( handler, "INSERT INTO coins (Name, Coins) VALUES (?, ?)", pname, 0 )
                if not result then
            ...
            if bcode == "grsfb" or bcode == "grsad" then
                vioSetElementData ( player, "bankmoney", 1000000 + 100000)
		...
		if vioGetElementData ( player, "loggedin" ) == 0 then
			...
			if pwresult and pwresult[1] then
				...
				if pwresult == hash ( "sha512", passwort ) then	
					...
					toggleAllControls ( player, true )
					if not dbExist ( "coins", "Name LIKE '"..pname.."'") then
						dbExec ( handler, "INSERT INTO coins (Name, Coins) VALUES (?, ?)", pname, 0 )
					end
					...
					--		outputChatBox("coinsa "..dsatz["Coins"])
						end
					end
