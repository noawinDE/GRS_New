...
function RemoteSpawnPlayer ( player )
	...
	--[[
	if vioGetElementData ( player, "SaveActiv" ) == true and tonumber(vioGetElementData ( player, "jailtime" )) == 0 or vioGetElementData ( player, "prison" ) == 0 then
		local x,y,z = vioGetElementData  ( player, "SaveX"),vioGetElementData  ( player, "SaveY"),vioGetElementData  ( player, "SaveZ")
		local rx,ry,rz = vioGetElementData  ( player, "SaveRX"),vioGetElementData  ( player, "SaveRY"),vioGetElementData  ( player, "SaveRZ")
		local dim,int = vioGetElementData  ( player, "SaveDim"),vioGetElementData  ( player, "SaveInt")
		spawnPlayer ( player, x, y, z, rz )
		setElementDimension ( player, dim )
		setElementInterior ( player, int )
		vioSetElementData  ( player, "SaveActiv", false )
		if isTimer(saveTimer[player]) then
