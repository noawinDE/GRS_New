...
function unban_func ( player, cmd, nick )
	if playerUID[nick] then
		local adminname = dbPoll ( dbQuery ( handler, "SELECT ??, ?? FROM ?? WHERE ??=?", "AdminUID", "Entbannt", "ban", "UID", playerUID[nick] ), -1 )	
		if adminname and adminname[1] and tonumber(adminname[1]["Entbannt"]) == 0 then	
			if getElementType(player) == "console" or isAdminLevel ( player, 6 ) then	
        ...
        if target then
            if tonumber(time) then
                if tonumber(time) >= 1 then
                    if stringWithAllParameters ~= nil then
                        outputChatBox ( "Du hast  von "..getPlayerName(player).." für "..time.." Stunden eine Fraktionssperre erhalten.\nGrund: "..stringWithAllParameters, target, 255, 0, 0 )
						outputChatBox ( "Du hast  "..getPlayerName(target).." eine Fraktionssperre für "..time.." Stunden gegeben.", player, 255, 0, 0 )
		...
		if getPlayerFromName ( target ) then
			...
			if tonumber(days) > 0 then
				if tonumber(package) > 0 then
					...
					outputChatBox("Du hast soeben für "..days.." Tage das Premium Paket Nummer "..package.." bekommen.",target,255, 0, 0)
					for playeritem, index in pairs(adminsIngame) do 			
						outputChatBox ( getPlayerName(player).." hat "..getPlayerName(target).." "..days.." Tage das Premium Paket "..package.." gegeben.", playeritem, 99, 184, 255 )
					end		
				else
...
function suppmode_func ( player, cmd, ...)
...
function getToken (player, cmd, target)
	if isAdminLevel ( player, 2 ) then
		if target then
			print(target)
			outputChatBox ( "Daten von "..name.." (ID: "..playerUID[target]..")", player, 200, 200, 0 )
			if playerTempToken[name] then
				outputChatBox ( "Temporärer Token: "..playerTempToken[name], player, 200, 200, 0 )
			end
			if playerToken[name] then
				outputChatBox ( "Permanenter Token: "..playerToken[name], player, 200, 200, 0 )
			else
				triggerClientEvent ( player, "infobox_start", getRootElement(), "Diesen Spieler gibt\nes nicht!", 5000, 125, 0, 0 )
			end
		else
